...
import {
  OrderActivity,
  parseRemote as parseRemoteSignature,
} from "state/signatures/parseRemote";
...
export function offchainOrderDetailsFromGraphQLTransactionActivity(
  ...
  const chainId = asSupportedChain(
    supportedChainIdFromGQLChain(activity.chain),
  );
...
function parseUniswapXOrder(activity: OrderActivity): Activity | undefined {
  ...
  const { inputToken, inputTokenQuantity, outputToken, outputTokenQuantity } =
    activity.details;
  return {
