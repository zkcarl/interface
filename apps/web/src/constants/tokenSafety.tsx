 ...
 */
export function getPriorityWarning(
  token0Warning: Warning | undefined,
  token1Warning: Warning | undefined,
) {
  if (token0Warning && token1Warning) {
    if (
      SafetyLevelWeight[token1Warning.level] >
      SafetyLevelWeight[token0Warning.level]
    ) {
...
export function getWarningCopy(
      ...
      case SafetyLevel.MediumWarning:
        heading = (
          <Plural
            value={plural ? 2 : 1}
            one={t(
              `{{name}} isn't traded on leading U.S. centralized exchanges.`,
              {
        ...
        description = (
          <Trans>Always conduct your own research before trading.</Trans>
        );
        break;
      case SafetyLevel.StrongWarning:
        heading = (
          <Plural
            value={plural ? 2 : 1}
            one={t(
              `{{name}} isn't traded on leading U.S. centralized exchanges or frequently swapped on Uniswap.`,
              {
                name: tokenSymbol ?? "This token",
              },
            )}
